...
bool Copter::ModePosHold::init(bool ignore_checks) {
	...
	// compute brake_gain
	poshold.brake_gain = (15.0f * (float) g.poshold_brake_rate + 95.0f)
			/ 100.0f;
...
#else
		if (ap.land_complete
				&& (takeoff_state.running || target_climb_rate > 0.0f)) {
#endif
	...
	if (ap.land_complete) {
		...
		if (runOpticalFlow(roll_out, pitch_out, target_roll, target_pitch)) {
			...
			runInnerRun(target_roll, target_pitch, target_yaw_rate,
					...
					vel_fw, vel_right, vel);
			attitude_control->input_euler_angle_roll_pitch_euler_rate_yaw(
					poshold.roll, poshold.pitch, target_yaw_rate,
					get_smoothing_gain());
...
void Copter::ModePosHold::runInnerRun(float &target_roll, float &target_pitch,
		float &target_yaw_rate, float &target_climb_rate,
		float &takeoff_climb_rate, float &brake_to_loiter_mix,
		float &controller_to_pilot_roll_mix,
		float &controller_to_pilot_pitch_mix, float &vel_fw, float &vel_right,
		const Vector3f& vel) {
	...
	if (poshold.roll_mode != POSHOLD_LOITER
			|| poshold.pitch_mode != POSHOLD_LOITER)
		poshold_get_wind_comp_lean_angles(poshold.wind_comp_roll,
				poshold.wind_comp_pitch);
	...
	case POSHOLD_PILOT_OVERRIDE:
		...
		if (is_zero(target_roll)
				&& (fabsf(poshold.pilot_roll) < 2 * g.poshold_brake_rate)) {
	...
	case POSHOLD_BRAKE_READY_TO_LOITER:
		...
		if (!poshold.braking_time_updated_roll) {
			...
			if (abs(poshold.brake_roll) >= poshold.brake_angle_max_roll) {
				...
				poshold.brake_timeout_roll = 1
						+ (uint16_t) (LOOP_RATE_FACTOR * 15L
								* (int32_t) (abs(poshold.brake_roll))
								/ (10L * (int32_t) g.poshold_brake_rate)); // the 1.2 (12/10) factor has to be tuned in flight, here it means 120% of the "normal" time.
				poshold.braking_time_updated_roll = true;
			}
		}

		// if velocity is very low reduce braking time to 0.5seconds
		if ((fabsf(vel_right) <= POSHOLD_SPEED_0)
	...
	case POSHOLD_CONTROLLER_TO_PILOT_OVERRIDE:
		...
		controller_to_pilot_roll_mix =
				(float) poshold.controller_to_pilot_timer_roll
						/ (float) POSHOLD_CONTROLLER_TO_PILOT_MIX_TIMER;

		// mix final loiter lean angle and pilot desired lean angles
		poshold.roll = poshold_mix_controls(controller_to_pilot_roll_mix,
				poshold.controller_final_roll,
				poshold.pilot_roll + poshold.wind_comp_roll);
		break;
	}
	...
	case POSHOLD_PILOT_OVERRIDE:
		...
		if (is_zero(target_pitch)
				&& (fabsf(poshold.pilot_pitch) < 2 * g.poshold_brake_rate)) {
			// initialise BRAKE mode
	...
	case POSHOLD_BRAKE_READY_TO_LOITER:
		...
		if (!poshold.braking_time_updated_pitch) {
			...
			if (abs(poshold.brake_pitch) >= poshold.brake_angle_max_pitch) {
				...
				// braking angle has started decreasing so re-estimate braking time
				poshold.brake_timeout_pitch = 1
						+ (uint16_t) (LOOP_RATE_FACTOR * 15L
								* (int32_t) (abs(poshold.brake_pitch))
								/ (10L * (int32_t) g.poshold_brake_rate)); // the 1.2 (12/10) factor has to be tuned in flight, here it means 120% of the "normal" time.
		...
		// if velocity is very low reduce braking time to 0.5seconds
		if ((fabsf(vel_fw) <= POSHOLD_SPEED_0)
	...
	case POSHOLD_CONTROLLER_TO_PILOT_OVERRIDE:
		...
		// calculate controller_to_pilot mix ratio
		controller_to_pilot_pitch_mix =
				(float) poshold.controller_to_pilot_timer_pitch
						/ (float) POSHOLD_CONTROLLER_TO_PILOT_MIX_TIMER;

		// mix final loiter lean angle and pilot desired lean angles
		poshold.pitch = poshold_mix_controls(controller_to_pilot_pitch_mix,
				poshold.controller_final_pitch,
				poshold.pilot_pitch + poshold.wind_comp_pitch);
		break;
	}
	...
	if (poshold.roll_mode == POSHOLD_BRAKE_READY_TO_LOITER
			&& poshold.pitch_mode == POSHOLD_BRAKE_READY_TO_LOITER) {
		...
		wp_nav->init_loiter_target(inertial_nav.get_position(),
				poshold.loiter_reset_I); // (false) to avoid I_term reset. In original code, velocity(0,0,0) was used instead of current velocity: wp_nav->init_loiter_target(inertial_nav.get_position(), Vector3f(0,0,0));
	...
	if (poshold.roll_mode == POSHOLD_BRAKE_TO_LOITER
			|| poshold.roll_mode == POSHOLD_LOITER) {
		...
		case POSHOLD_BRAKE_TO_LOITER:
			...
			brake_to_loiter_mix = (float) poshold.brake_to_loiter_timer
					/ (float) POSHOLD_BRAKE_TO_LOITER_TIMER;

			// calculate brake_roll and pitch angles to counter-act velocity
			poshold_update_brake_angle_from_velocity(poshold.brake_roll,
					vel_right);
			poshold_update_brake_angle_from_velocity(poshold.brake_pitch,
					-vel_fw);
			...
			poshold.roll = poshold_mix_controls(brake_to_loiter_mix,
					poshold.brake_roll + poshold.wind_comp_roll,
					wp_nav->get_roll());
			poshold.pitch = poshold_mix_controls(brake_to_loiter_mix,
					poshold.brake_pitch + poshold.wind_comp_pitch,
					wp_nav->get_pitch());
...
void Copter::ModePosHold::poshold_update_pilot_lean_angle(
		float &lean_angle_filtered, float &lean_angle_raw) {
	// if raw input is large or reversing the vehicle's lean angle immediately set the fitlered angle to the new raw angle
	if ((lean_angle_filtered > 0 && lean_angle_raw < 0)
			|| (lean_angle_filtered < 0 && lean_angle_raw > 0)
			|| (fabsf(lean_angle_raw) > POSHOLD_STICK_RELEASE_SMOOTH_ANGLE)) {
		...
		if (lean_angle_filtered > 0) {
			...
			lean_angle_filtered -= MAX(
					(float) lean_angle_filtered * POSHOLD_SMOOTH_RATE_FACTOR,
					MAX(1, g.poshold_brake_rate / LOOP_RATE_FACTOR));
...
int16_t Copter::ModePosHold::poshold_mix_controls(float mix_ratio,
		int16_t first_control, int16_t second_control) {
	mix_ratio = constrain_float(mix_ratio, 0.0f, 1.0f);
	return (int16_t) ((mix_ratio * first_control)
			+ ((1.0f - mix_ratio) * second_control));
}
...
void Copter::ModePosHold::poshold_update_brake_angle_from_velocity(
		int16_t &brake_angle, float velocity) {
	float lean_angle;
	...
	if (velocity >= 0) {
		lean_angle = -poshold.brake_gain * velocity
				* (1.0f + 500.0f / (velocity + 60.0f));
	} else {
		lean_angle = -poshold.brake_gain * velocity
				* (1.0f + 500.0f / (-velocity + 60.0f));
	}
	...
	brake_angle = constrain_int16((int16_t) lean_angle,
			brake_angle - brake_rate, brake_angle + brake_rate);

	// constrain final brake_angle
	brake_angle = constrain_int16(brake_angle, -g.poshold_brake_angle_max,
			g.poshold_brake_angle_max);
}
...
void Copter::ModePosHold::poshold_update_wind_comp_estimate() {
	...
	if (is_zero(poshold.wind_comp_ef.x)) {
		...
		poshold.wind_comp_ef.x = (1.0f - TC_WIND_COMP) * poshold.wind_comp_ef.x
				+ TC_WIND_COMP * accel_target.x;
	}
	if (is_zero(poshold.wind_comp_ef.y)) {
		...
		// low pass filter the position controller's lean angle output
		poshold.wind_comp_ef.y = (1.0f - TC_WIND_COMP) * poshold.wind_comp_ef.y
				+ TC_WIND_COMP * accel_target.y;
	}
}
...
void Copter::ModePosHold::poshold_get_wind_comp_lean_angles(int16_t &roll_angle,
		int16_t &pitch_angle) {
	// reduce rate to 10hz
	...
	roll_angle = atanf(
			(-poshold.wind_comp_ef.x * ahrs.sin_yaw()
					+ poshold.wind_comp_ef.y * ahrs.cos_yaw()) / 981)
			* (18000 / M_PI);
	pitch_angle = atanf(
			-(poshold.wind_comp_ef.x * ahrs.cos_yaw()
					+ poshold.wind_comp_ef.y * ahrs.sin_yaw()) / 981)
			* (18000 / M_PI);
}
